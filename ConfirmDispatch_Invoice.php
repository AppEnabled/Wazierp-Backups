...
/* Remove location code from order number */
if ($_GET['OrderNumber']) {
    $OrderNumberOnly = substr($_GET['OrderNumber'], 5);
}
if (!isset($_GET['OrderNumber']) && !isset($_SESSION['ProcessingOrder'])) {
    ...
    exit;
} elseif (isset($_GET['OrderNumber']) and $OrderNumberOnly > 0) {

    unset($_SESSION['Items']->LineItems);
    unset($_SESSION['Items']);

    $_SESSION['ProcessingOrder'] = $_GET['OrderNumber'];
    //echo "order number isssssss " . $_SESSION['ProcessingOrder'] . "<br>";
...
//echo "sql is " . $OrderHeaderSQL . "<br>";
    ...
    if (DB_num_rows($GetOrdHdrResult) == 1) {
        //echo "only one <br>";
        //echo "<br>commm is  " . $myrow['comments'] . "<br>";
        $myrow = DB_fetch_array($GetOrdHdrResult);
        if($myrow['quotation'] == $Quotations){
            ...
            echo "<script>jQuery.facebox('One cannot invoice a quotation, please convert quotation to a sales order to invoice.');</script>";
            exit();
        }
        ...
        $_SESSION['Items']->PaymentTerms = $myrow['paymentterms'];
...
//echo "<br>comments  ssisisisis  " . $_SESSION['Items']->Comments . "<br>";
        ...
        $ErrMsg = _('The details of the customer selected') . ': ' .$_SESSION['Items']->DebtorNo . ' ' . _('cannot be retrieved because');
        ...
        if ($_SESSION['CheckCreditLimits'] > 0) {
            ...
            $_SESSION['Items']->CreditAvailable = GetSaleCreditAvailable($_SESSION['Items']->DebtorNo,$_GET['OrderNumber'], $db);
            ...
            if ($_SESSION['CheckCreditLimits'] == 1 AND $_SESSION['Items' . $identifier]->CreditAvailable <= 0) {
                ...
                if (in_array(15, $_SESSION['AllowedPageSecurityTokens'])) {
                    prnMsg(_('Credit Of...') . ' ' . $_SESSION['Items' . $identifier]->CreditAvailable . ' ' . _(' '), 'warn');
                    exit;
                }
        ...
        $LineItemsSQL = 'SELECT stkcode,
			    ...
			    (salesorderdetails.unitprice*(1-salesorderdetails.discountpercent)) - (stockmaster.materialcost + stockmaster.labourcost + stockmaster.overheadcost) AS gp,
			    ((salesorderdetails.unitprice*(1-salesorderdetails.discountpercent)) - (stockmaster.materialcost + stockmaster.labourcost + stockmaster.overheadcost))/(salesorderdetails.unitprice*(1-salesorderdetails.discountpercent))as lowgp,
			    salesorderdetails.unitprice*(1-salesorderdetails.discountpercent) as sellingprice
        ...
        if (db_num_rows($LineItemsResult) > 0) {
            ...
            while ($myrow = db_fetch_array($LineItemsResult)) {
                $GPPercent = 0;
                $GPPercent = (int)(($myrow['sellingprice']-$myrow['standardcost'])*100/$myrow['sellingprice']) ;
                if($GPPercent <= $_POST['GPMin']){
                    echo $myrow['lowgp'] ."----------".$_POST['GPMin'] ."<br />";
                    $DisplayGPPercent = number_format(($myrow['gp']*100)/$myrow['sellingprice'],1);
                    $DisplayGP = number_format($myrow['gp'],2);
                    //echo "<tr><td>".$myrow['stkcode']." is selling at a low gp of ".$DisplayGPPercent."%</td></tr>";
                    $Messages .= "<tr><td class=tdc><font color=red><u>".$myrow['stkcode']. ", of  GP: ".$DisplayGP. " is selling at a low GP percentage of..".$DisplayGPPercent ." %</u></font></td></tr>";
                    //continue;
                }
                //echo "<br>Pack row is " . $myrow['packdesc'] . "<br>";
                $_SESSION['Items']->add_to_cart($myrow['stkcode'], $myrow['quantity'], $myrow['description'], $myrow['unitprice'], $myrow['discountpercent'], $myrow['units'], $myrow['volume'], $myrow['kgs'], 0, $myrow['mbflag'], $myrow['actualdispatchdate'], $myrow['qtyinvoiced'], $myrow['discountcategory'], $myrow['controlled'], $myrow['serialised'], $myrow['decimalplaces'], $myrow['narrative'], 'No', $myrow['orderlineno'], $myrow['taxcatid'], '', $myrow['itemdue'], $myrow['poline'], $myrow['standardcost'], '', '', $myrow['packdesc']
                ); /* NB NO Updates to DB */
...
echo '<form action="' . $_SERVER['PHP_SELF'] . '?' . SID . '" method=post name="DispatchForm">';
/***************************************************************/
/*************GP settings *************************************/
...
foreach ($_SESSION['Items']->LineItems as $LnItm) {
    ...
    if ($LnItm->Controlled == 1) {
        ...
        echo '<td class="number"><input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" tabindex="' . $j . '" type="text" class="number" name="' . $LnItm->LineNumber . '_QtyDispatched" maxlength=12 size=12 value="' . $LnItm->QtyDispatched . '"></td>';
    }
    ...
    foreach ($LnItm->Taxes AS $Tax) {
        ...
        echo '<input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="text" class="number" name="' . $LnItm->LineNumber . $Tax->TaxCalculationOrder . '_TaxRate" maxlength=4 size=4 value="' . $Tax->TaxRate * 100 . '">';
        $i++;
        if ($Tax->TaxOnTax == 1) {
    ...
    echo '<td class="number">' . $LnItm->Packdesc . '</td>';
...
if ($_SESSION['Items']->Any_Already_Delivered() == 1 and (!isset($_SESSION['Items']->FreightCost) or $_POST['ChargeFreightCost'] == 0)) {
    echo '<td colspan=2 align=right>' . _('Charge Freight Cost inc Tax') . '</td>
		<td><input tabindex=' . $j . ' onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="text" class="number" size="10" maxlength="12" name="ChargeFreightCost" VALUE="0"></td>';
    $_SESSION['Items']->FreightCost = 0;
} else {
    echo '<td colspan=2 align=right>' . _('Charge Freight Cost inc Tax') . '</td>
		<td><input tabindex=' . $j . ' onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="text" class="number" size="10" maxlength="12" name="ChargeFreightCost" VALUE="' . $_SESSION['Items']->FreightCost . '"></td>';
...
foreach ($_SESSION['Items']->FreightTaxes as $FreightTaxLine) {
    ...
    echo '<input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="text" class="number" name="FreightTaxRate' . $FreightTaxLine->TaxCalculationOrder . '" maxlength="4" size="4" VALUE="' . $FreightTaxLine->TaxRate * 100 . '">';
...
if (isset($_POST['ProcessInvoice']) && $_POST['ProcessInvoice'] != "") {
    ...
    $ResultEx = DB_query($SQLEx, $db, $ErrMsg);
    if(DB_num_rows($ResultEx)  > 0)
    {
        $NotInvoiced = false;
    }else{
        $NotInvoiced = true;;
...
if($NotInvoiced){
    echo '<link href="' . $rootpath . '/facebox/src/facebox.css" media="screen" rel="stylesheet" type="text/css"/>';
    echo '<script src="' . $rootpath . '/facebox/src/facebox.js" type="text/javascript"></script>';
    echo "<script>jQuery.facebox('This order is already invoived, please check this.');</script>";
    exit();
}
    ...
    //echo "terms is " . $_SESSION['Items']->PaymentTerms . "<br>";
    ...
    echo $_SESSION['ProhibitNegativeStock'];
    if ($_SESSION['ProhibitNegativeStock'] == 1) { // checks for negative stock after processing invoice
        ...
        foreach ($_SESSION['Items']->LineItems as $OrderLine) {
            ...
            $sqlCheckduplicate ="SELECT SUM(quantity) AS chqty
				FROM salesorderdetails
                                WHERE orderno ='" . $_SESSION['ProcessingOrder'] . "'
                                AND stkcode = '" . $OrderLine->StockID . "'
                                ";
            $Resultsduplicate = DB_query($sqlCheckduplicate, $db, $ErrMsg);
//            if (DB_num_rows($Resultsduplicate) > 1) {
//                if ($PreviousItem != $OrderLine->StockID) {
//                    $PreviousItem = $OrderLine->StockID;
                   $myrow = DB_fetch_array($Resultsduplicate);
                   $SumedQuantity = $myrow['chqty'];
//                    }
//                }
//            }
            ...
            //$PreviousItem = array('ItemCode'=>$OrderLine->StockID ,'Quantity'=>$CheckNegRow['quantity']);
            if ($CheckNegRow['allownegative'] != 1) {
                //|| $CheckNegRow['quantity'] < $SumedQuantity
                if ($CheckNegRow['mbflag'] == 'B' OR $CheckNegRow['mbflag'] == 'M') {
                    if ($CheckNegRow['quantity'] < $OrderLine->QtyDispatched || $CheckNegRow['quantity'] < $SumedQuantity ) {
                        ...
                        $ErrorMsgArray[] = 'Invoicing the selected order would result in negative stock. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected. ' . $OrderLine->StockID . ' ' . $CheckNegRow['description'] . ' - ' . 'Negative Stock Prohibited';
                        $NegativesFound = true;
                    }
                } elseif ($CheckNegRow['mbflag'] == 'A') {

//                    if ($SumedQuantity == 0) {
//                        $locstockQuantiy = $OrderLine->QtyDispatched;
//                    } else {
//                        $locstockQuantiy = $SumedQuantity;
//                    }

                    /* Now look for assembly components that would go negative OR $CheckNegRow['quantity'] < $OrderLine->Quantity */
                    $SQL = "SELECT bom.component,
							   stockmaster.description,
							   locstock.quantity-(" . $SumedQuantity . "*bom.quantity) AS qtyleft
						FROM bom
						INNER JOIN locstock
                    ...
                    while ($NegRow = DB_fetch_array($Result)) {
                        if ($NegRow['qtyleft'] < 0) {
                            ...
                            $ErrorMsgArray[] = 'Invoicing the selected order would result in negative stock for a component of an assembly item on the order. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected.' . $NegRow['component'] . ' ' . $NegRow['description'] . ' - ' . 'Negative Stock Prohibited';
                            $NegativesFound = true;
    ...
    foreach ($_SESSION['Items']->LineItems as $OrderLine) {
        ...
        } elseif ($OrderLine->QOHatLoc < $OrderLine->Quantity AND ($OrderLine->MBflag == 'B' OR $OrderLine->MBflag == 'M') OR $OrderLine->Price < $OrderLine->StandardCost && $myrow['negative'] != "Y") {
            if ($OrderLine->Price < $OrderLine->StandardCost && $myrow['negative'] != "Y") {
                ...
                echo '<link href="' . $rootpath . '/facebox/src/facebox.css" media="screen" rel="stylesheet" type="text/css"/>';
                echo '<script src="' . $rootpath . '/facebox/src/facebox.js" type="text/javascript"></script>';
                echo "<script>jQuery.facebox('Sell price for $OrderLine->StockID at R $PriceDisplay is below cost price of $Cost.');</script>";
    ...
    $NextInvoiceNo = GetNextTransNoTenderType(10, $db, $TypeNoField);
    $InvoiceNo = $_SESSION['UserStockLocation'] . $NextInvoiceNo;
    $_SESSION['CurrentInvoice'] = $InvoiceNo;
    ...
    $OverallinvTotal = ($_SESSION['Items']->total);
    ...
    //echo "<br>Trans id is " . $DebtorTransID . "<br>";
    ...
    foreach ($_SESSION['Items']->LineItems as $OrderLine) {
        ...
        if ($_POST['BOPolicy'] == 'CAN') {
            ...
            $SQL = "UPDATE salesorderdetails
				SET quantity = quantity - " . ($OrderLine->Quantity - $OrderLine->QtyDispatched) . ",
                                negative = 'N'
                                WHERE orderno = '$_SESSION[ProcessingOrder]' AND stkcode = '" . $OrderLine->StockID . "'";
        ...
        if ($OrderLine->QtyDispatched != 0 AND $OrderLine->QtyDispatched != "" AND $OrderLine->QtyDispatched) {
            ...
            $ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The sales 11111order detail record could not be updated because');
            $DbgMsg = _('The following SQL to update the sales order detail record was used');
            $Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, true);
            ...
            if ($myrow[0] > 0) { /* Update the existing record that already exists */
                ...
                $InvoicingTransType = "CUST-INVOICE";
        ...
        if ($OrderLine->Price < $OrderLine->StandardCost) {
            ...
            undercost($_SESSION['UserID'], $InvoiceNo, $OrderLine->StockID, $OrderLine->Price, $OrderLine->StandardCost);
        }
    ...
    DB_Txn_Commit($db);
    $TenderTypeDebtor = $_SESSION['Items']->DebtorNo;
    $TenderTypeTotal = number_format($TaxTotal + ($_SESSION['Items']->total + $_POST['ChargeFreightCost']), 2);
    $TenderTypeTotal = str_replace(",", "", $TenderTypeTotal);
    unset($_SESSION['Items']->LineItems);
    ...
    if ($_SESSION['Items']->PaymentTerms != "CA") {
    ...
    $OrderNumber = $_SESSION['Items']->OrderNo;

    echo '<a href="' . $rootpath . '/SelectSalesOrder.php?' . SID . '">' . _('Select another order for invoicing') . '</a><br><br>';
    echo '<a href="' . $rootpath . '/SelectOrderItems.php?' . SID . 'NewOrder=Yes">' . _('Sales Order Entry') . '</a><br><br>';

    /* Get terms for debtor, if it is not cash dont show receive payment on screen PDT */
    if ($_SESSION['Items']->PaymentTerms == "CA") {
        echo '<a href="' . $rootpath . '/CustomerReceiptTenderType.php?' . SID . 'Debtor=' . $TenderTypeDebtor . '&InvNumber=' . $InvoiceNo . '&TenderTotal=' . $TenderTypeTotal . '&NewReceipt=Yes&OrderNumber=' . $OrderNumber . '">' . _('Receive Payment') . '</a></div><br>';
    }
    unset($_SESSION['Items']);
    ...
    if (!isset($_POST['InvoiceText'])) {
        ////echo "<br>comments is  " . $_SESSION['Items']->Comments . "<br>";
        $_POST['InvoiceText'] = $_SESSION['Items']->Comments;
    }
    $j++;
    echo '<table><tr>
		<td>' . _('Date On Invoice') . ':</td>
	<td><input tabindex=' . $j . ' onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="text" maxlength="10" size="15"
    ...
    echo '<tr>
		...
		<td><input tabindex=' . $j . ' onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type=text maxlength=15 size=15 name=Consignment value="' . $_POST['Consignment'] . '"></td>
	</tr>';
    $j++;
    echo '<tr>
...
echo '</form>';
echo '<script type="text/javascript" language="JavaScript">document.DispatchForm.elements[5].focus();</script>';
include('includes/footer.inc');
?>