...
//    echo $_POST['DeliveryDate']. '<br />';
//    echo $_SESSION['Items' . $identifier]->DeliveryDate . "<br />";
//    exit();
...
if (isset($OK_to_PROCESS) and $OK_to_PROCESS == 1 && $_SESSION['ExistingOrder'] == "0") {
    ...
    echo "Not existing order " . $_SESSION['ExistingOrder'] . "<br />";
    echo " " . $_SESSION['Items' . $identifier]->Quotation . "<br />";
    ...
    if ($_SESSION['ActiveTenderType'] == "Y" && $_SESSION['Items' . $identifier]->Quotation != 1) {
        echo "Check active tender type .. type =" . $_SESSION['ActiveTenderType'] . "<br />";
        ...
        echo "This is a qotation- > prfeix is QT, order number " . $OrderNo . " <br />";
    }
    echo "Order numnber " . $OrderNo . ", created <br />";

       /*Avoid Error Messages/ Seems like our Quotation losses value*/
        if(empty($_SESSION['Items' . $identifier]->Quotation)){
            if(preg_match('/SO/', $OrderNo)){
                $_SESSION['Items' . $identifier]->Quotation = 0;
            }elseif(preg_match('/QT/', $OrderNo)){
    ...
    echo "Inserted  header details, for" . $OrderNo . "  <br />";

    if ($_SESSION['ActiveTenderType'] == "Y" && $_SESSION['Items' . $identifier]->Quotation != 1) {
        echo "Temp sales Orders insert.. <br />";
        ...
        foreach ($_SESSION['Items' . $identifier]->LineItems as $StockItem) {
            ...
            $ErrMsg = _('Unable to add the temp sales order line - line 632');
            // $DbgMsg = _('The SQL that failed was');
            $DbgMsg = _('The following SQL to temp Sales Order - lines was used');
            $run_temp = DB_query($TempLineItemsSQL, $db, $ErrMsg, $DbgMsg, true);
        }
    }
    echo "Sales Order Details  insert.. <br />";
    ...
    foreach ($_SESSION['Items' . $identifier]->LineItems as $StockItem) {
        ...
        //echo "<br>sql line 433 " . $LineItemsSQL . "<br>";
        $ErrMsg = _('Unable to add the sales order line - line 669');
        // $DbgMsg = _('The SQL that failed was');
        $DbgMsg = _('The following SQL to Sales Order - lines was used - line 669');
    ...
    if (count($_SESSION['AllowedPageSecurityTokens']) > 1) {
        ...
        if ($_POST['Quotation'] == 0 && !$_SESSION['Items' . $identifier]->CashPaymentTerms == 'CA') { /* then its not a quotation its a real order */
            ...
            echo '<p><img src="' . $rootpath . '/css/' . $theme . '/images/printer.png" title="' . _('Print') . '" alt="">' . ' ' . '<a  target="_blank" href="' . $rootpath . '/PrintCuttingSlip.php?' . SID . 'ModifyOrderNumber=' . $OrderNo . '">' . _('Print cutting Slip') . ' (' . _('Laser') . ')' . '</a>';
...
//echo 1;
        ...
        echo '<br/><img src="' . $rootpath . '/css/' . $theme . '/images/printer.png" title="' . _('Print') . '" alt="">' . ' ' . '<a  target="_blank" href="' . $rootpath . '/PrintCuttingSlip.php?' . SID . 'ModifyOrderNumber=' . $OrderNo . '">' . _('Print cutting Slip') . ' (' . _('Laser') . ')' . '</a>';
    ...
    if ($_POST['ConvertOrder'] == 1 && $_SESSION['Items' . $identifier]->Quotation == 1) {// Convert quotation to sales orders
        ...
        $HeaderSQL = 'UPDATE salesorders
			  SET debtorno = ' . "'" . $_SESSION['Items' . $identifier]->DebtorNo . "'" . ',
				...
				deladd6 = ' . "'" . DB_escape_string($_SESSION['Items' . $identifier]->DelAdd6) . "'" . ',';
        if ($_POST['dateopt'] == "txtdate" && $_POST['inputedate'] != '') {
            $HeaderSQL .= ' ordernumofdays =' . $_POST['inputedate'] . ',';
        } else {
            //deliverydate = '."'" . FormatDateForSQL(DB_escape_string($_SESSION['Items'.$identifier]->DeliveryDate)) . "'".',
            $HeaderSQL .= '  deliverydate = ' . "'" . FormatDateForSQL(DB_escape_string($_SESSION['Items' . $identifier]->DeliveryDate)) . "'" . ',';
        }
...
if (in_array(2, $_SESSION['AllowedPageSecurityTokens'])) {
    ...
    foreach ($_SESSION['Items' . $identifier]->LineItems as $StockItem) {
        ...
        //echo "<br>Pack desc is " . $StockItem[Taxes]->Packdesc . "<br>";
