...
If (isset($_POST['PrintPDF'])
	...
	if ($_POST['CustomerSpecials']==_('Customer Special Prices Only')){
		...
		$SQL = "SELECT prices.typeabbrev,
			...
			ORDER BY prices.currabrev,
				stockmaster.categoryid,
				stockmaster.stockid";
        //AND ((prices.price*(1-".$_POST['discount'].")) - (stockmaster.materialcost + stockmaster.labourcost + stockmaster.overheadcost))/(prices.price*(1-".$_POST['discount'].")) <=" . ($_POST['margins']/100) . "
	...
	include('includes/PDFPriceListPageHeaderCost.inc');
	$CurrCode ='';
	$Category = '';
	$CatTot_Val=0;
	While ($PriceList = DB_fetch_array($PricesResult,$db)){
        ...
        if ($PriceList['price']!=0){
            ...
            $GPPercent = (int)(($PriceList['price']-$PriceList['standardcost'])*100/$PriceList['price']) ;
        } else {
            $DisplayGPPercent = 0;
        }

        if ($_POST['ShowGPPercentages']=='Yes' && $GPPercent <= $_POST['margins']){

        }else{
            if(isset($_POST['lowGProducts'])){
                continue;
            }
        }
		...
		$LeftOvers = $pdf->addTextWrap(140,$YPos,260,$FontSize,$PriceList['description']);
		$DisplayStandardCost = number_format($PriceList['standardcost'],2);
        $DisplayActualCost  = number_format($PriceList['actualcost'],2);
		$LeftOvers = $pdf->addTextWrap(340,$YPos,60,$FontSize,$DisplayStandardCost);// was actual
        $LeftOvers = $pdf->addTextWrap(380,$YPos,60,$FontSize,$DisplayActualCost);
		if ($_POST['CustomerSpecials']=='Customer Special Prices Only'){
			...
			if ($PriceList['branchcode']!=''){
				$LeftOvers = $pdf->addTextWrap(440,$YPos,130,$FontSize,$PriceList['brname'],'left');
			} else {
				$LeftOvers = $pdf->addTextWrap(440,$YPos,130,$FontSize,_('All'),'left');
			}
		...
		if ($_POST['ShowGPPercentages']=='Yes'){
			$LeftOvers = $pdf->addTextWrap(440,$YPos,20,$FontSize,$DisplayGPPercent, 'right');
		}

		$LeftOvers = $pdf->addTextWrap(460,$YPos,40,$FontSize,$DisplayUnitPrice, 'right');
		$DisplayUnitPriceVAT = number_format($PriceList['price']*1.14,2);		
		$LeftOvers = $pdf->addTextWrap(500,$YPos,40,$FontSize,$DisplayUnitPriceVAT, 'right');
...
/*Print out the category totals */
	...
	/*if $FromCriteria is not set then show a form to allow input	*/
		...
		echo '</select></td></tr>';
        echo "<tr><td>" . _('GP less/Equal to %') . "</td>
		<td><input type='text' name='margins' value='".$_POST['margins']."'></td></tr><tr>";
       /* echo "<tr><td>" . _('Discount %') . "</td>
		<td><input type='text' name='discount' value='".$_POST['discount']."'></td></tr><tr>";*/
		...
		echo '</select></td></tr>';
        echo '<tr><td>' . _('Tick to show low GP products') . '<td><input type="checkbox" name="lowGProducts"></tr>';
