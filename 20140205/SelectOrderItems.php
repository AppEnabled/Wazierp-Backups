...
if (isset($_POST['BarCode']))
{
  ...
  unset ($NewBItem_array);
    ...
    $_POST['order_items'] = $_POST['BarCode'];
    	if (!isset($NewBItem_array)) {
		$NewBItem_array = array();
	}
//echo "5".$_POST['BarCode'].$_POST['Location']."location". $_POST['order_items']." <br>";
    ...
    //echo "<b> This line <b><br>".$myrow['stockid'];
    /*Because we scanning an item, we intial quantities to one..*/
   $Units = 1;
   $PackSize = 1;
   $QuantityLine = 1;
   /*if (in_array($myrow['stockid'],$NewItem_array))
   {
        $QuantityLine++;
        $NewItem_array[] = array( $myrow['stockid'], $Units, $PackSize, $QuantityLine);
   }
    else*/
...
$count = 0;
    ...
    //	echo "key is " . $key . "<br>";
        ...
        if (strstr($key, "itm") && $value > 0) {
        	...
        	//echo "<br>q line " . $QuantityLine . "<br>";
        	//echo "<br>stock id is " . $Stockid . "<br>";
        	//$NewItem_array[$Stockid]['Quantity'] = $QuantityLine;
        //	print_r($NewItem_array);
            //$NewItem_array[substr($key, 3)] = trim($value);
        	//$itemname = substr($key, 3);
    	...
    	/*if (strstr($key, "calc")) {
    		if ($value['0'] != 0) {
    			echo "calc 0 " . $value['0'] . "<br>";
    			echo "calc 1 " . $value['1'] . "<br>";
    			echo "calc 2 " . $value['2'] . "<br>";
    		}
...
$count = ++$count;
   ...
   // echo "<br>q line isss " . $QuantityLine . "<br>";
    	$NewItem_array[] = array($Stockid, $Units, $PackSize, $QuantityLine);
    		unset($Stockid);
    		unset($QuantityLine);
    		unset($Units);
    		unset($PackSize);
	...
	//print_r($NewItem_array);
	//print_r($NewItem_array);
	//$_SESSION['Packsize'] = $PackSizeArray;
...
/*Get Items From Ajax-Dynamic(Tamelo Douglas)*/
...
if (isset($_GET['NewOrder'])) {
	/* Check if there is untendered invoices PDT 7July10 */
	$SQL = 'SELECT
			...
			AND debtorsmaster.paymentterms="CA"
			AND debtortrans.user = "' . $_SESSION['UserID'] . '"';

	$ErrMsg = _('Could not retrieve open invoices because)');
	$Result = DB_query($SQL,$db,$ErrMsg);
	if ($myrowtt = mysql_num_rows($Result) > 0) {
		prnMsg(_('There is outstanding invoices to tender.'), 'error');
...
//echo "<br>sql line 205 is " . $OrderHeaderSQL . "<br>";
    ...
    if (DB_num_rows($GetOrdHdrResult) == 1) {
        ...
        $_SESSION['Items' .  $identifier]->CustRefVia  = $myrow['custContact'];
        ...
        $_SESSION['Items' . $identifier]->DeliverTo = stripslashes($myrow['deliverto']);
        $_SESSION['Items' . $identifier]->DeliveryDate = ConvertSQLDate($myrow['deliverydate']);
        $_SESSION['Items' . $identifier]->DelAdd1 = stripslashes($myrow['deladd1']);
        $_SESSION['Items' . $identifier]->DelAdd2 = stripslashes($myrow['deladd2']);
        $_SESSION['Items' . $identifier]->DelAdd3 = stripslashes($myrow['deladd3']);
        $_SESSION['Items' . $identifier]->DelAdd4 = stripslashes($myrow['deladd4']);
        $_SESSION['Items' . $identifier]->DelAdd5 = stripslashes($myrow['deladd5']);
        $_SESSION['Items' . $identifier]->DelAdd6 = stripslashes($myrow['deladd6']);
        ...
        //$_SESSION['Items' . $identifier]->PackDesc = $myrow['pacdesc'];
...
//echo "<br>sql line 522 is " . $sql . "<br>";
      ...
      $SQLA = "SELECT creditlimit
				FROM debtorsmaster
				WHERE debtorno='" . $_POST['Select'] . "' 
                                AND debtorno LIKE '%CASH%'";
        $ErrMsg = _('Something has gone wrong');
        echo  $SQLA . '<br />';
        $AtResult = DB_query($SQLA, $db, $ErrMsg, $DbgMsg);
        if(DB_num_rows($AtResult) != 1)
        {
...
if ($_SESSION['RequireCustomerSelection'] == 1
	...
	<td><input tabindex=1 onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="Text" name="CustKeywords" size=20	maxlength=25></td>
	<td><h2><b><?php echo _('OR'); ?></b></h2></td>
	<td><h5><?php echo _('Part of the Customer Branch Code'); ?>:</h5></td>
	<td><input tabindex=2 onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="Text" onkeyup="ajax_showOptions(this,'getCustomer',event)" name="CustCode" size=15	maxlength=18 autocomplete=off></td>
	<td><h2><b><?php echo _('OR'); ?></b></h2></td>
	<td><h5><?php echo _('Part of the Branch Phone Number'); ?>:</h5></td>
	<td><input tabindex=3 onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="Text" name="CustPhone" size=15	maxlength=18></td>
	</tr>
	</table>
    ...
    if (isset($_POST['CancelOrder'])) {
        ...
        if ($OK_to_delete == 1) {
            ...
            if ($_SESSION['ExistingOrder'] != "0") {
                ...
                $SQLCHCK  = "SELECT order_  FROM debtortrans
                                              WHERE order_ = '".$_SESSION['ExistingOrder']."'";
                $chckResult = DB_query($SQLCHCK , $db, $ErrMsg);
                
                /***********************************************************************************************************/
                if(DB_num_rows($chckResult) > 0){
                    
                    $SQL = 'DELETE FROM salesorderdetails WHERE salesorderdetails.orderno = "'.$_SESSION['ExistingOrder'].'"
                                                                    AND salesorderdetails.qtyinvoiced = 0
...
//echo "<br>location 799 is " . $_POST['Location'] . "<br>";
    	...
    	if (!isset($_POST['Location'])) {
    		$_POST['Location'] = $_SESSION['UserStockLocation'];
    	}
	...
	//echo "<br>sql line 913 is " . $SQL . "<br>";
        if (isset($_POST['Next'])) {
    	...
    	//echo "<br>rows are " . DB_num_rows($SearchResult) . "<br>";
        if (DB_num_rows($SearchResult) == 1 && $_POST['order_items']) {
//echo "<br>Now at line 963 ";
...
//echo "<br>dis is " . $_POST['Discount_' . $OrderLine->LineNumber] . "<br>";
        ...
        while ($i <= $_SESSION['QuickEntries'] and isset($_POST['part_' . $i]) and $_POST['part_' . $i] != '') {
            ...
            if (DB_num_rows($KitResult) == 0) {
                ...
                if ($myrow['mbflag'] == 'K') {
                    ...
                    prnMsg(_('Phantom assemblies cannot be sold, these items exist only as bills of materials used in other manufactured items. The following item has not been added to the order:') . ' ' . $NewItemQty['0'], 'warn');
                } else {
        ...
        foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
            ...
            if (isset($_POST['Quantity_' . $OrderLine->LineNumber])) {
                ...
                if (!Is_Date($_POST['ItemDue_' . $OrderLine->LineNumber])) {
                    prnMsg(_('An invalid date entry was made for ') . ' ' . $NewItemQty['0'] . ' ' . _('The date entry') . ' ' . $ItemDue . ' ' . _('must be in the format') . ' ' . $_SESSION['DefaultDateFormat'], 'warn');
                    // Attempt to default the due date to something sensible?
                    $_POST['ItemDue_' . $OrderLine->LineNumber] = DateAdd (Date($_SESSION['DefaultDateFormat']), 'd', $_SESSION['Items' . $identifier]->DeliveryDays);
                }
            	//code to prevent sell under cost
            	$Dontsell = false;
            	$PricePerUnit = $_POST['Price_' . $OrderLine->LineNumber] * (1 - ($_POST['Discount_' . $OrderLine->LineNumber] / 100));
            	$Minimumsellprice = $OrderLine->StandardCost;
            	//echo "<br>Flag is " . $OrderLine->MBflag . "<br>";

            	//echo "<br>Min sell is " . $Minimumsellprice . "<br>";
            	//echo "<br>Price is " . $PricePerUnit . "<br>";
            	/*if ($PricePerUnit < $Minimumsellprice && $OrderLine->MBflag != 'Z') {
            		prnMsg(_('Sell price of ') . $OrderLine->StockID . (' is below cost price of ') . number_format($OrderLine->StandardCost, 2) . ('. Only prices higher than cost allowed. Please contact Corrine or Sean to correct this.'), 'error');
            		$Cost = number_format($OrderLine->StandardCost, 2);
            		//echo "<script=\"javascript\">jQuery.facebox('something cool')</script>";
            		echo "<script>jQuery.facebox('Sell price for $OrderLine->StockID is below cost price of $Cost. Only prices higher than cost allowed. Please contact Corrine or Sean to correct this.');</script>";
    ...
    /*end of if its a new item */
    if (isset($NewItem_array)&& isset($_POST['order_items'])&& !$_POST['BarCode']) {
    ...
    elseif (isset($NewBItem_array)&& isset($_POST['order_items']) && isset($_POST['BarCode'])) {
   ...
   // }
    ...
    if (count($_SESSION['Items' . $identifier]->LineItems) > 0) {
        ...
        foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
        	foreach ($PackSizeArray as $key => $value){
        		if ($key == $OrderLine->StockID) {
        			//echo "value is " . $value['unit'] . "<br>";

        			$OrderLine->PackUnits = $value['unit'];
        			//$OrderLine->PackOf = $value['of'];
        			$OrderLine->PackPack = $value['pack'];
        			//$OrderLine->PackDesc = $value['packdesc'];
        			//echo "<br><br>Stock id is the same<br><br>";
        		}
        	}
			//echo "<br><br>Stock id is " . $OrderLine->StockID . "<br>";
            $LineTotal = $OrderLine->Quantity * $OrderLine->Price * (1 - $OrderLine->DiscountPercent);
			//$PackUnits = $OrderLine->PackUnits;
        	//$PackSizeQty = $OrderLine->Packsize;
            ...
            if ($OrderLine->QOHatLoc < $OrderLine->Quantity AND ($OrderLine->MBflag == 'B' OR $OrderLine->MBflag == 'M') OR $OrderLine->Price < $OrderLine->StandardCost) {
            	/* Sell Price is lower than cost price */
            	if ($OrderLine->Price < $OrderLine->StandardCost) {
            		...
            		echo "<script>jQuery.facebox('Sell price for $OrderLine->StockID at $PriceDisplay is below cost price of $Cost. Only prices higher than cost allowed. Please contact Corrine or Sean to correct this.');</script>";
            	}
            ...
            if ($_SESSION['Items' . $identifier]->DefaultPOLine == 1) { // show the input field only if required
                echo '<td><input tabindex=1 onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type=text name="POLine_' . $OrderLine->LineNumber . '" size=20 maxlength=20 value=' . $OrderLine->POLine . '></td>';
            } else {
            ...
            echo '<td><a target="_blank" href="' . $rootpath . '/StockStatus.php?' . SID . 'identifier=' . $identifier . '&StockID=' . $OrderLine->StockID . '&DebtorNo=' . $_SESSION['Items' . $identifier]->DebtorNo . '">' . $OrderLine->StockID . '</a></td>
				<td align="center">' . $OrderLine->ItemDescription . '</td>
				<td align="center">' . $OrderLine->PackUnits . '</td>
				<td align="center">X</td>
				<td align="center">' . $OrderLine->Packsize . '</td>
';

            echo '<td><input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" class="number" onKeyPress="return restrictToNumbers(this, event)" tabindex=2 type=text name="Quantity_' . $OrderLine->LineNumber . '" size=6 maxlength=6 value=' . $OrderLine->Quantity . '>';
            ...
            if (in_array(2, $_SESSION['AllowedPageSecurityTokens'])) {
                ...
                echo '<td><input onFocus="this.select();setBKColor(event);" class="number" onKeyPress="return restrictToNumbers(this, event)" onblur="reSetBKColor(event);" type=text name="Price_' . $OrderLine->LineNumber . '" size=16 maxlength=16 value=' . $OrderLine->Price . '></td>
					<td><input onFocus="this.select();setBKColor(event);" class="number" onKeyPress="return restrictToNumbers(this, event)" onblur="reSetBKColor(event);" type=text name="Discount_' . $OrderLine->LineNumber . '" size=5 maxlength=4 value=' . ($OrderLine->DiscountPercent * 100) . '></td>
					<td><input onFocus="this.select();setBKColor(event);" class="number" onKeyPress="return restrictToNumbers(this, event)" onblur="reSetBKColor(event);" type=text name="GPPercent_' . $OrderLine->LineNumber . '" size=3 maxlength=40 value=' . $OrderLine->GPPercent . '></td>';
            } else {
			...
			//echo "<br>Line items " . print_r($_SESSION['Items' . $identifier]->LineItems) . "<br>";
            echo '<td><input onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type=text class="date" alt="' . $_SESSION['DefaultDateFormat'] . '" name="ItemDue_' . $OrderLine->LineNumber . '" size=10 maxlength=10 value=' . $LineDueDate . '></td>';
            if(isset($_GET['ModifyOrderNumber']))
            {
    ...
    if (isset($_POST['PartSearch']) && $_POST['PartSearch'] != '' || !isset($_POST['QuickEntry'])) {
		...
		<td><input tabindex=2 onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="Text" name="Keywords" size=20 maxlength=25 value="<?php if (isset($_POST['Keywords'])) echo $_POST['Keywords']; ?>"></td></tr>
		<tr><td></td>
		<td align="right"><b><?php echo _('OR'); ?> </b></td><td><b><?php echo _('Enter partial'); ?> <?php echo _('Stock Code'); ?>:</b></td>
		<td><input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" onkeyup="ajax_showOptions(this,'getItem',event)" tabindex=3 type="Text" name="StockCode" size=15 maxlength=18 value="<?php if (isset($_POST['StockCode'])) echo $_POST['StockCode']; ?>" autocomplete=off></td>
		</tr>
                <tr>
                 <td></td><td align="right"><b><?php echo _('OR'); ?> </b></td><td><b><?php echo _('Enter '); ?> <?php echo _('Bar-Code'); ?>:</b></td>
		<td><input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);"  tabindex=4 type="Text" name="BarCode" size=20 maxlength=18 value="<?php ?>" ></td>
		</tr>
		...
		?><td align="right"><b><?php echo _('AND'); ?> </b></td><td><b><?php echo _('Select'); ?> <?php echo _('Location'); ?>:</b></td>
		...
		<input onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" tabindex=5 type=submit name="QuickEntry" value="<?php echo _('Use Quick Entry'); ?>">
        ...
        if (in_array(2, $_SESSION['AllowedPageSecurityTokens'])) {
            ...
            echo '</b>' . $_SESSION['Items' . $identifier]->Location;
...
if (isset($SearchResult)) {
            ...
            echo '<td style="text-align:center" colspan=6><input type="hidden" name="order_items" value=1><input tabindex=' . number_format($j + 8) . ' name="OrderAnItem "type="submit" value="' . _('Order') . '"></td>';
...
*/
                ...
                printf('<td>%s</font></td>
					...
					<td><font size=1><input class="number"  tabindex=' . number_format($j + 7) . ' type="textbox" size=6 id="packresult' . $count . '" name="itm' . $myrow['stockid'] . '" onFocus="CalcUnits(' . $count . ');setBKColor(event);this.select();" onblur="reSetBKColor(event);">
					</td>
					</tr>',
                    $myrow['stockid'],
                    $myrow['description'],
    ...
    /*end of PartSearch options to be displayed */
        ...
        for ($i = 1;$i <= $_SESSION['QuickEntries'];$i++) {
            ...
            if ($_SESSION['Items' . $identifier]->DefaultPOLine > 0) {
                echo '<td><input onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="text" name="poline_' . $i . '" size=21 maxlength=20></td>';
            }
            echo '<td><input type="text" name="part_' . $i . '" size=21 maxlength=20></td>
						<td><input onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="text" name="qty_' . $i . '" size=6 maxlength=6></td>
						<td><input onfocus="setBKColor(event);" onblur="reSetBKColor(event);" type="text" class="date" name="itemdue_' . $i . '" size=25 maxlength=25
    ...
    if ($_SESSION['Items' . $identifier]->ItemsOrdered >= 1) {
        $calcelOder = 35;
        if($_SESSION['Items'.$identifier]->Quotation !=1){
            /*cancel by super user, only sales Orders, no quotations*/
        if(in_array($calcelOder,$_SESSION['AllowedPageSecurityTokens']))
        {
