...
if (isset($_GET['AllocTrans'])) {
	...
	//$_SESSION['Alloc']->TransDate = ConvertSQLDate($myrow['trandate']);
        $_SESSION['Alloc']->TransDate = $myrow['trandate'];
	echo "<br>Amount is " . $_SESSION['Alloc']->TransAmt . "<br>";
	echo "<br>Debtorno is " . $_SESSION['Alloc']->DebtorNo . "<br>";
	...
	$Result = DB_query($SQL,$db);
	echo "<br>sql line 280 is " . $SQL . "<br>";
	...
	echo "<br>Second total " . $myrow['total'] . "<br>";
	DB_free_result($Result);
	...
	$SQL= "SELECT debtortrans.id,
			...
			ORDER BY debtortrans.trandate";
	echo "<br>sql2 is " . $SQL . "<br>";
	$Result=DB_query($SQL,$db);
	echo "<br>post is " . $_POST['AllocTrans'] . "<br>";
	while ($myrow=DB_fetch_array($Result))
	{
	...
	echo "<br>and that amount " . $myrow['amt'] . "<br>";
	DB_free_result($Result);
}
	...
	elseif (isset($_GET['DebtorNo'])) // Page called with customer code
	{
		...
		$SQL = "SELECT debtortrans.id,
				...
				debtortrans.rate,
				debtortrans.ovamount+debtortrans.ovgst+debtortrans.ovfreight as total,
		...
		while ($myrow = DB_fetch_array($result))
		{
			if ($myrow['ovamount'] == 0 && $myrow['ovdiscount'] <> 0) {
				$myrow['typename'] = "Discount";
				$myrow['total'] = $myrow['ovdiscount'];
				echo "<br>ova " . $myrow['ovamount'] . '-' .$myrow['transno'] ."<br>";
			}
			echo "<tr>
	...
	else  // Page called with no parameters
	{
		...
		while ($myrow = DB_fetch_array($result))
		{
            ...
            if($myrow['ovdiscount'] > 0.01){
                //Show the discount row for the trasaction...
                echo "<tr>
					<td>Discount</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
					<td class=number>" . number_format($myrow['ovdiscount'],2) . "</td>
					<td class=number></td>";
                    echo '<td></td></tr>';
            }
