...
// Only quotes allowed in this file
...
/* finally write the order header to the database and then the order line details */
	...
	if (count($_SESSION['AllowedPageSecurityTokens'])>1){
		...
		if ($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '<div class="centre"><p><img src="'.$rootpath.'/css/'.$theme.'/images/reports.png" title="' . _('Order') . '" alt="">' . ' ' . '<a href="' . $rootpath . '/PDFQuotation.php?' . SID .'identifier='.$identifier . '&QuotationNo=' . $OrderNo . '">'. _('Print Quotation') .'</a>';
                        echo '<div class="centre"><p>Beta : Quotation With Images</p></div>';
                        echo '<div class="centre"><p><img src="'.$rootpath.'/css/'.$theme.'/images/reports.png" title="' . _('Order') . '" alt="">' . ' ' . '<a href="' . $rootpath . '/PDFImgQuotation.php?' . SID .'identifier='.$identifier . '&QuotationNo=' . $OrderNo . '">'. _('Print Quotation With Images') .'</a>';

		}
		///echo '<p><img src="'.$rootpath.'/css/'.$theme.'/images/sales.png" title="' . _('Order') . '" alt="">' . ' ' . '<a href="'. $rootpath .'/SelectOrderItemsCash.php?' . SID .'identifier='.$identifier . '&NewOrder=Yes">'. _('Add Sales Order') .'</a></div>';
	} else {
...
echo "Existing Quotation ";*/
	...
	$Result = DB_Txn_Begin($db);
        if($_SESSION['Items'.$identifier]->Quotation == 0 )/* if to convert to an order....*/
        {
...
//echo 1;
		...
		if ($StockItem->Quantity == $StockItem->QtyInv){
			...
			$Completed = 0;
               ...
               if ($myrow['stkcode']== $StockItem->StockID)
               {
                   echo $myrow[' stkcode'];
 ...
 else 
 {
                $LineItems2 =  'INSERT INTO salesorderdetails (orderlineno,orderno,
					...
					'."'" . $_POST['Pack' . $StockItem->LineNumber] . "'".'
                                ...
                                $Upd_LineItemResult = DB_query( $LineItems2,$db);
	...
	if($_SESSION['Items'.$identifier]->Quotation ==1)
        {
	  prnMsg(_('Quotation number') .' ' . $OrderNs . ' ' . _('has been Updated'),'success');
        }
        else
            prnMsg(_('Order number') .' ' . $OrderNs . ' ' . _('has been Updated'),'success');
 ...
 */
	...
	exit;
        ...
        else if ( $_POST['Quotation'] == 1 && $_SESSION['Items'.$identifier]->Quotation == 1  ){
...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/inventory.png" title="' . _('Delivery') . '" alt="">' . ' ' . _('Quotation Delivery Details');
...
echo '<tr>
	...
	<td><input class="date" alt="'.$_SESSION['DefaultDateFormat'].'" onFocus="this.select();setBKColor(event);" onblur="reSetBKColor(event);" type="Text" size=15 maxlength=14 name="DeliveryDate" value="' . $_SESSION['Items'.$identifier]->DeliveryDate . '"></td>
	</tr>';
...
echo "" .$_SESSION['Items'.$identifier]->CustRefVia;
 $sql = 'SELECT  code,Description FROM custReferences';
            ...
            $result = Db_query($sql,$db,$ErrMsg);
     echo "<option value='".$_SESSION['Items'.$identifier]->CustRefVia."'>" . _('--Select Contact Via--');
...
echo '<tr>
	<td></td>
	<td><textarea name="Comments" id ="Comments" cols=31 rows=5>' . html_entity_decode($_SESSION['Items'.$identifier]->Comments) .'</textarea></td>
</tr>';
...
if ($_SESSION['ExistingOrder']=="0"){
    //new one
	echo '<br><br><input type=submit name="ProcessOrder" value="' . _('Create Quotation') . '">';
        }
        else if($_SESSION['ExistingOrder']!="0" && $_SESSION['Items'.$identifier]->Quotation==0){

          // echo '<br><br><input type=submit name="Cquotation" value="' . _('Convert-Quotation') . '">';
           echo '<br><br><input type=submit name="ProcessOrder" value="' . _('Convert-Invoice') . '">';
           echo '<br><br><input type=submit name="MakeRecurringOrder" VALUE="' . _('Create Recurring Order') . '">';
        }
            else {
       // echo '<br><br><input type=submit name="ProcessOrder" value="' . _('Order -Invoice') . '">';
	echo '<br><input type=submit name="ProcessOrder" VALUE="' . _('Invoice Quotation') . '">';
}
