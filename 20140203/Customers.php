...
if (isset($_POST['submit'])) {
	...
	if ($myrow[0]>0 and isset($_POST['New'])) {
		...
		$Errors[$i] = 'DebtorNo';
		$i++;
	} elseif (strlen($_POST['CustName']) > 60 OR strlen($_POST['CustName'])==0) {
		$InputError = 1;
		prnMsg( _('The customer name must be entered and be sixty characters or less long'),'error');
		$Errors[$i] = 'CustName';
		$i++;
		...
		if (!isset($_POST['New'])) {
			...
			if ($myrow[0] == 0) {
			  $sql = "UPDATE debtorsmaster SET
					...
					settlementdiscount=" . $_POST['settlementdiscount'] . ",
				    basicrebate=" . $_POST['basicrebate'] . ",
				    advertisingrebate=" . $_POST['advertisingrebate'] . ",
					bulkrebate=" . $_POST['bulkrebate'] . ",
					...
					emailstatements='" . $_POST['EmailStatements'] . "',
					regnum='" . $_POST['RegNum'] . "'
			...
			$sql = "INSERT INTO debtorsmaster (
				...
				VALUES ('" . $_POST['DebtorNo'] ."',
					...
					" . $_POST['CreditLimit'] . ",
				    " . $_POST['settlementdiscount'] . ",
				    " . $_POST['basicrebate'] . ",
				    " . $_POST['advertisingrebate'] . ",
				     " . $_POST['bulkrebate'] . ",
					'" . $_POST['SalesType'] . "',
					'" . $_POST['AddrInvBranch'] . "',
					'" . $_POST['TaxRef'] . "',
...
//	unset($_POST['SalesType']);
	...
	unset($_POST['Statements']);
	unset($_POST['EmailStatements']);
...
/*If the page was called without $_POST['DebtorNo'] passed to page then assume a new customer is to be entered show a form with a Debtor Code field other wise the form showing the fields with the existing entries against the customer will show for editing with only a hidden DebtorNo field*/
	...
	echo '<tr><td>' . _('Customer Name') . ':</td>
		<td><input tabindex=2 type="Text" name="CustName" size=42 maxlength=60></td></tr>';
	echo '<tr><td>' . _('Show on invoice?') . ':</td>
		<td><input tabindex=3 type="checkbox" name="check"></td></tr>';
	echo '<tr><td>' . _('Address Line 1 (Street)') . ':</td>
		<td><input tabindex=4 type="Text" name="Address1" size=42 maxlength=40></td></tr>';
	echo '<tr><td>' . _('Address Line 2 (Suburb/City)') . ':</td>
		<td><input tabindex=5 type="Text" name="Address2" size=42 maxlength=40></td></tr>';
	echo '<tr><td>' . _('Address Line 3 (State/Province)') . ':</td>
		<td><input tabindex=6 type="Text" name="Address3" size=42 maxlength=40></td></tr>';
	echo '<tr><td>' . _('Address Line 4 (Postal Code)') . ':</td>
		<td><input tabindex=7 type="Text" name="Address4" size=42 maxlength=40></td></tr>';
	echo '<tr><td>' . _('Address Line 5') . ':</td>
		<td><input tabindex=8 type="Text" name="Address5" size=22 maxlength=20></td></tr>';
	echo '<tr><td>' . _('Address Line 6') . ':</td>
		<td><input tabindex=9 type="Text" name="Address6" size=17 maxlength=15></td></tr>';
	echo '<tr><td>' . _('Tel') . ':</td>
	<td><input tabindex=9 type="Text" name="Tel" size=17 maxlength=15></td></tr>';
...
// Show Customer Type drop down list
       ...
       if (DB_num_rows($result)==0){
               ...
               echo '<tr><td colspan=2>' . prnMsg(_('No Customer types/price lists defined'),'error') . '</td></tr>';
                ...
                echo '<tr><td>' . _('Customer Type') . ':</td>
                        <td><select tabindex=10 name="typeid">';
	...
	echo '<tr><td>' . _('Customer Since') . ' (' . $_SESSION['DefaultDateFormat'] . '):</td><td><input tabindex=11 type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="ClientSince" value="' . $DateString . '" size=12 maxlength=10></td></tr>';
	echo '<tr><td>' . _('Discount Percent') . ':</td>
		<td><input tabindex=12 type="textbox" class="number" name="Discount" value=0 size=5 maxlength=4></td></tr>';
	echo '<tr><td>' . _('Discount Code') . ':</td>
		<td><input tabindex=13 type="text" name="DiscountCode" size=3 maxlength=2></td></tr>';
	echo '<tr><td>' . _('Payment Discount Percent') . ':</td>
		<td><input tabindex=14 type="textbox" class ="number" name="PymtDiscount" value=0 size=5 maxlength=4></td></tr>';
	echo '<tr><td>' . _('Credit Limit') . ':</td>
		<td><input tabindex=15 type="text" class="number" name="CreditLimit" value=' . $_SESSION['DefaultCreditLimit'] . ' size=16 maxlength=14></td></tr>';
	echo '<tr><td>' . _('Tax Reference') . ":</td>
		<td><input tabindex=16 type='Text' name='TaxRef' size=22 maxlength=20></td></tr>";
	echo '<tr><td>' . _('Registration Number') . ":</td>
		<td><input tabindex=17 type='Text' name='RegNum' size=22 maxlength=20></td></tr>";
    echo '<tr><td>' . _('Settlement discount') . ":</td><td><input type='text' name='settlementdiscount' size=20 maxlength=20></td></tr>";
    echo '<tr><td>' . _('Basic rebate') . ":</td><td><input type='text' name='basicrebate' size=20 maxlength=20></td></tr>";
    echo '<tr><td>' . _('Advertising rebate') . ":</td><td><input type='text' name='advertisingrebate' size=20 maxlength=20></td></tr>";
    echo '<tr><td>' . _('Bulk rebate') . ":</td><td><input type='text' name='bulkrebate' size=20 maxlength=20></td></tr>";
	...
	if (DB_num_rows($result)==0){
		...
		echo '<tr><td>' . _('Payment Terms') . ':</td>
			<td><select tabindex=18 name="PaymentTerms">';
	...
	echo '<tr><td>' . _('Credit Status') . ':</td><td><select tabindex=19 name="HoldReason">';
	...
	if (DB_num_rows($result)==0){
		...
		echo '<tr><td>' . _('Customer Currency') . ':</td><td><select tabindex=20 name="CurrCode">';
		while ($myrow = DB_fetch_array($result)) {
	...
	echo '<tr><td>' . _('Customer PO Line on SO') . ":</td><td><select tabindex=21 name='CustomerPOLine'>";
		echo '<option selected value=0>' . _('No');
		echo '<option value=1>' . _('Yes');
	echo '</select></td></tr>';

	echo '<tr><td>' . _('Invoice Addressing') . ":</td><td><select tabindex=22 name='AddrInvBranch'>";
	...
	/*---------------------------------------------------------------------*/
	/*    Statements	Email, etc - Added by Dylan 29/02/2012 				   */
	/*---------------------------------------------------------------------*/
	
	
	echo '<tr><td>' . _('Customer Receives Statements?') . ":</td><td><select tabindex=23 name='Statements'>";
		echo '<option selected value=0>' . _('No');
		echo '<option value=1>' . _('Yes');
	echo '</select></td></tr>';
	
	echo '<tr><td>' . _('Email Customer Statements?') . ":</td><td><select tabindex=24 name='EmailStatements'>";
		echo '<option selected value=0>' . _('No');
		echo '<option value=1>' . _('Yes');
	echo '</select></td></tr>';
	...
	if ($DataError ==0){
		echo "<div class='centre'><input tabindex=25 type='Submit' name='submit' value='" . _('Add New Customer') . "'><br><input tabindex=24 type=submit action=RESET VALUE='" . _('Reset') . "'></div>";
	}
	echo '</form>';
...
//DebtorNo exists - either passed when calling the form or from the form itself
	...
	if (!isset($_POST['New'])) {
		...
		$_POST['Statements'] = $myrow['statements'];
		$_POST['EmailStatements'] = $myrow['emailstatements'];
		$_POST['RegNum'] = $myrow['regnum'];
        $_POST['settlementdiscount'] = $myrow['settlementdiscount'];
		$_POST['basicrebate'] = $myrow['basicrebate'];
		$_POST['advertisingrebate'] = $myrow['advertisingrebate'];
		 $_POST['bulkrebate'] = $myrow['bulkrebate'];


		//echo "<br>ch is  " . $checked . "<br>";
	...
	echo '<tr><td>' . _('Customer Name') . ':</td>
		<td><input ' . (in_array('CustName',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="CustName" value="' . $_POST['CustName'] . '" size=42 maxlength=60></td></tr>';
	echo '<tr><td>' . _('Address Line 1 (Street)') . ':</td>
	...
	echo '<tr><td>' . _('Tel') . ':</td>
		<td><input ' . (in_array('Tel',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="Tel" size=42 maxlength=40 value="' . $_POST['Tel'] . '"></td></tr>';
    echo '<tr><td>&nbsp;</td><td>&nbsp;</td></tr>';
    echo '<tr><td>&nbsp;</td><td>&nbsp;</td></tr>';
    echo '<tr><td>&nbsp;</td><td>&nbsp;</td></tr>';
    echo '<tr><td>&nbsp;</td><td>&nbsp;</td></tr>';
...
// Select Customer types for drop down list for SELECT/UPDATE
	...
	echo '<tr><td>' . _('Registration Number') . ':</td>
		<td><input type="Text" name="RegNum" size=23 maxlength=20  value="' . $_POST['RegNum'] . '"></td></tr>';
    echo '<tr><td>' . _('Settlement discount') . ":</td><td><input type='text' name='settlementdiscount' size=20maxlength=20 VALUE='" . $_POST['settlementdiscount'] . "'></td></tr>";
    echo '<tr><td>' . _('Basic rebate') . ":</td><td><input type='text' name='basicrebate' size=20maxlength=20 VALUE='" . $_POST['basicrebate'] . "'></td></tr>";
    echo '<tr><td>' . _('Advertising rebate') . ":</td><td><input type='text' name='advertisingrebate' size=20 maxlength=20 VALUE='" . $_POST['advertisingrebate'] . "'></td></tr>";
    echo '<tr><td>' . _('Bulk rebate') . ":</td><td><input type='text' name='bulkrebate' size=20 maxlength=20 VALUE='" . $_POST['bulkrebate'] . "'></td></tr>";
	$result=DB_query('SELECT terms, termsindicator FROM paymentterms',$db);
	...
	echo '<tr><td>' . _('Customer Receives Statements?') . ":</td>
		<td><select name='Statements'>";
	if ($_POST['Statements']==0){
		echo '<option selected value=0>' . _('No');
		echo '<option value=1>' . _('Yes');
	} else {
		echo '<option value=0>' . _('No');
	...
	echo '<tr><td>' . _('Email Customer Statements?') . ":</td>
		<td><select name='EmailStatements'>";
	if ($_POST['EmailStatements']==0){
		echo '<option selected value=0>' . _('No');
		echo '<option value=1>' . _('Yes');
	} else {
		echo '<option value=0>' . _('No');
	...
	echo '<tr>
			...
			<th>' . _('Email') . '</th>
			<th>' . _('Notes') . '</th>
			<th>' . _('Receive Statement Email?') . '</th>
			<th>' . _('Edit') . '</th>
