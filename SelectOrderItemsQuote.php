...
// This file is for quotes only
$_SESSION['Items' . $identifier]->Quotation = 1;
// Needs to be set again on line 153 due to unsetting
...
if ($_SESSION['RequireCustomerSelection'] == 1
    ...
    ' ' . _('Enter a Quotation') . ' : ' . _('Search for the Customer Branch.') . '</p>';
    echo '<div class="page_help_text">' . _('Quotations are placed against the Customer Branch. A Customer may have several Branches.') . '</div>';
    ...
    if (isset($_POST['CancelOrder'])) {
		...
		// Manually set that it is a quote
		$_SESSION['Items' . $identifier]->Quotation = 1;
...
//echo "<br>dis is " . $_POST['Discount_' . $OrderLine->LineNumber] . "<br>";
    ...
    if (isset($_POST['DeliveryDetails'])) {
        ...
        echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/DeliveryDetailsQuote.php?' . SID . 'identifier=' . $identifier . '">';
        prnMsg(_('You should automatically be forwarded to the entry of the delivery details page') . '. ' . _('if this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
            '<a href="' . $rootpath . '/DeliveryDetailsQuote.php?' . SID . 'identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
        exit;
    }
       ...
       // foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
        foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
            ...
            if($GPPercent <= $_POST['GPMin']){
                ...
                //of  GP: ".$DisplayGP. "
                $Messages .= "<tr><td class=tdc><font color=red><u>".$OrderLine->StockID. ",  is selling at a low GP percentage of..".$DisplayGPPercent ." %</u></font></td></tr>";
                //continue;
            }
        }

        if( $Messages != ""){
            /*Eceho all the items with low gp..  Only if message not empty*/
