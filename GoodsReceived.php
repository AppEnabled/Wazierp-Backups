...
if (isset($_GET['PONumber']) and $_GET['PONumber'] == "" and !isset($_SESSION['PO'])) {
    ...
    foreach ($_SESSION['PO']->LineItems as $Line) {
        ...
        $Price = $_POST['Price_' . $Line->LineNo];
        if (!is_numeric($RecvQty)) {
            $RecvQty = 0;
        }
        /* This should stop receiving at zero value */
        if ($Price == 0 || (double) $Price <= 0.01) {
            $Messages .= "<tr><td class=tdc><font color=red><u>You Can not receive stock at zero(0) value..".$Line->StockID ."- R".$Line->Price .", Please Check with management</u></font></td></tr>";
        ...
        echo "Line items " . $Line->LineNo . "---" . $Line->StockID . "Price update is...." . $Line->Price . "<br />";
        $_SESSION['PO']->LineItems[$Line->LineNo]->ReceiveQty = $RecvQty;
        $_SESSION['PO']->LineItems[$Line->LineNo]->Price = $Price;
        $SQLCHECK = "SELECT materialcost FROM stockmaster WHERE stockid = '" . $Line->StockID . "'";
        $Result = DB_query($SQLCHECK, $db);
        $myrow = DB_fetch_array($Result);
        if($myrow[0] > 0)
        if ((($Line->Price - $myrow[0]) / $myrow[0]) * 100 > 10 || (($myrow[0] - $Line->Price) / $Line->Price) * 100 > 10) {
            $PriceChange = 1;
            $OldPrice = $myrow[0];
            $Messages .= "<tr><td class=tdc><font color=red><u>Stock Code $Line->StockID  price will change from  $OldPrice to $Line->Price</u></font></td></tr>";
        }
    }
    if ($Messages != "" || $PriceChange == 1) {
        echo '<link href="' . $rootpath . '/facebox/src/facebox.css" media="screen" rel="stylesheet" type="text/css"/>';
        echo '<script src="' . $rootpath . '/facebox/src/facebox.js" type="text/javascript"></script>';
        echo "<script>jQuery.facebox('$Messages');</script>";
    }
...
if (count($_SESSION['PO']->LineItems) > 0) {
    foreach ($_SESSION['PO']->LineItems as $LnItm) {
        ...
        $TotalPrice = $DisplayPrice * $DisplayQtyRec;
        ...
        // echo "<br>qty is " . $LnItm->ReceiveQty . "<br>";
        ...
        echo '<td><input type=text class=number name="Price_' . $LnItm->LineNo . '" maxlength=10 size=10 onKeyPress="return restrictToNumbers(this, event)" onFocus="return setTextAlign(this, ' . "'" . 'right' . "'" . ')" value="' . $LnItm->Price . '"></td>';
        // echo '<td class=number><font size=2><input' . $DisplayPrice . '</td>';
...
if ($SomethingReceived == 0 AND isset($_POST['ProcessGoodsReceived'])) {
    ...
    $NextGrnNo = GetNextTransNoTenderType(13, $db, $TypeNoField);
    $LocGrn = $_SESSION['UserStockLocation'] . $NextGrnNo;
    
    
    while ($myrow = DB_fetch_array($Result)) {
        // echo "Line 260 glcode is " . $myrow['glcode'] . "<br>";
    ...
    foreach ($_SESSION['PO']->LineItems as $OrderLine) {
        ...
        $SQLCHECK = "SELECT materialcost FROM stockmaster WHERE stockid = '" . $OrderLine->StockID . "'";
        $Result = DB_query($SQLCHECK, $db);
        $myrow = DB_fetch_array($Result);
        if($myrow[0] > 0)
        if ((($OrderLine->Price - $myrow[0]) / $myrow[0]) * 100 > 10 || (($myrow[0] - $OrderLine->Price) / $OrderLine->Price) * 100 > 10) {
            $PriceChange = 1;
            $OldPrice = $myrow[0];
            $message .= "<tr><td class=tdc><font color=blue><u>Stock Code $OrderLine->StockID  price : changed from  $OldPrice to  $OrderLine->Price</u></font></td></tr>";
        }
    }
    //|| (($myrow[0]-$POLine->Price)/$POLine->Price)*100 < 10
    if ($message != "" || $PriceChange == 1) {
        echo '<link href="' . $rootpath . '/facebox/src/facebox.css" media="screen" rel="stylesheet" type="text/css"/>';
        echo '<script src="' . $rootpath . '/facebox/src/facebox.js" type="text/javascript"></script>';
        ...
        if ($OrderLine->ReceiveQty != 0 AND $OrderLine->ReceiveQty != '' AND isset($OrderLine->ReceiveQty)) {
            ...
            if ($OrderLine->ReceiveQty >= ($OrderLine->Quantity - $OrderLine->QtyReceived)) {
                $SQL = "UPDATE purchorderdetails SET
							...
							unitprice=" . $_SESSION['PO']->LineItems[$OrderLine->LineNo]->Price . ",
							completed=1
           ...
           if ($UnitCost == 0 || (double) $UnitCost <= 0.01) {
                $Messages .= "<tr><td class=tdc><font color=red><u>You Can not receive stock at zero(0) value..".$OrderLine->StockID. "- R ".$OrderLine->Price."/".$_SESSION['PO']->ExRate .",standard cost (".$UnitCost.") cannot be zero. Please Check with management/Exit and reload again</u></font></td></tr>";
                echo '<link href="' . $rootpath . '/facebox/src/facebox.css" media="screen" rel="stylesheet" type="text/css"/>';
                echo '<script src="' . $rootpath . '/facebox/src/facebox.js" type="text/javascript"></script>';
                echo "<script>jQuery.facebox('$Messages');</script>";
            ...
            // echo "grn " . $GRN . "<br>";
            // echo "1 " . $OrderLine->PODetailRec . "<br>";
            // echo "2 " . $OrderLine->StockID . "<br>";
            // echo "3 " . $OrderLine->ItemDescription . "<br>";
            // echo "4 " . $_POST['DefaultReceivedDate'] . "<br>";
            // echo "5 " . $OrderLine->ReceiveQty . "<br>";
            // echo "6 " . $_SESSION['PO']->SupplierID . "<br>";
            // echo "7 " . $CurrentStandardCost . "<br>";
            /* Get next GRN number for loccation PDT - 22/6/2010 */
            // $TypeNoField = "typeno" . $_SESSION['UserStockLocation'];
            // $NextGrnNo = GetNextTransNoTenderType(13, $db, $TypeNoField);
            // $LocGrn = $_SESSION['UserStockLocation'] . $NextGrnNo;
            ...
            /*Put a purchase/actual price in actualcost (stockmaster)*/
            $SQLlast = "UPDATE stockmaster SET actualcost=".$OrderLine->Price."
                                 WHERE stockid='" . $OrderLine->StockID . "'";
            $Results = DB_query($SQLlast, $db, $ErrMsg, $DbgMsg, true);
            /* Get Existing  Price */
//         $SQLMatcost = "SELECT materialcost 
//			FROM stockmaster
//			WHERE stockid='" . $OrderLine->StockID . "'";
//                $ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The standard cost of the item being received cannot be retrieved because');
//                $DbgMsg = _('The following SQL to retrieve the standard cost was used');
//                $Results = DB_query($SQLMatcost, $db, $ErrMsg, $DbgMsg, true);
//
//             $myrowMatCost = DB_fetch_row($Results);
            /* Quantity received is != 0 */
//          if($myrowMatCost[0] != $UnitCost && $OrderLine->StockID !="")
//          {
//            $_POST['GrvReference'] = "Goods Received";
//            $SqlGRV = "INSERT INTO stockcosts (stockid,type,transno,
//						loccode,
//						trandate,
//						prd,
//						reference,
...
//							'" . $_SESSION['UserID'] . "')";
//                                        echo   $SqlGRV ."<br />";
//					$ErrMsg =  _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('Stock Costs records could not be inserted because');
//					$DbgMsg = _('The following SQL to insert the stock-costs records was used');
//					$ResultGRV = DB_query($SqlGRV,$db,$ErrMsg,$DbgMsg,true);
//                                      
//                        }   
        }//if quantity received not equal to zero...  
    }
    ...
    echo '<br>' . _('GRN number') . ' ' . $LocGrn . ' ' . _('has been processed') . '<br>';
    echo '<br><a href=PDFGrn.php?GRNNo=' . $GRN . '&PONo=' . $PONo . '&LocGrn=' . $LocGrn . '>' . _('Print this Goods Received Note (GRN)') . '</a><br><br>';
