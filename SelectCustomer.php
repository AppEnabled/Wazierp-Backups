...
and !isset($_POST['CustPhone']))) {
	//echo "<br>select is " . $_POST['Select'] . "<br>";

	if ($_POST['Select']!=""){
		//echo "<br>Now at line 242<br>";
		$SQL = "SELECT brname, phoneno, branchcode, prikey FROM custbranch WHERE debtorno='" . $_POST['Select'] . "'
		AND custbranch.defaultlocation = '" . $_SESSION['UserStockLocation'] . "'";
		$_SESSION['CustomerID'] = $_POST['Select'];
		//echo "<br>Cus id is " . $_SESSION['CustomerID'] . "<br>";
	} else {
		$SQL = "SELECT debtorsmaster.name, debtorsmaster.tel, custbranch.branchcode, custbranch.phoneno, custbranch.prikey, custbranch.defaultlocation FROM
		debtorsmaster, custbranch WHERE
		custbranch.debtorno='" . $_SESSION['CustomerID'] . "'
		AND custbranch.defaultlocation = '" . $_SESSION['UserStockLocation'] . "'
		AND	debtorsmaster.debtorno = custbranch.debtorno";
		//echo "<br>Now at line 250<br>";
	}
	...
	echo '<a href="' . $rootpath . '/CustomerOrderEnq.php?CustomerID=' . $_SESSION['CustomerID'] . '">' . _('Customer Order Inquiries') . '</a><br>';
    echo '<a href="' . $rootpath . '/CustomerPurchaseHistory.php?CustomerID=' . $_SESSION['CustomerID'] . '">' . _('Customer Purchase History') . '</a><br>';
	...
	if ($Prikey != 0) {
		echo '<a target=new href="http://www.bzto.bz/bztobz8/RepActivity/ReviewTaskClient.aspx?prikey=' . $Prikey . '">' . _('BZtoBZ') . '</a><br>';
	}
...
if (isset($result)) {
	...
	if (!isset($_POST['CSV'])) {
		...
		$TableHeader = '<tr>
				<th>' . _('Head Office Code') . '</th>
				<th>' . _('Branch Name') . '</th>
				<th>' . _('Head Office Name') . '</th>
				<th>' . _('Branch Name') . '</th>
				<th>' . _('Prikey') . '</th>
				<th>' . _('Contact') . '</th>
				<th>' . _('Type') . '</th>
				<th>' . _('Branch Phone') . '</th>
				<th>' . _('Branch Fax') . '</th>
			</tr>';
	...
	if (DB_num_rows($result)<>0){
		...
		while (($myrow=DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			printf("<td><font size=1><input type=submit name='Select' VALUE='%s'><br>%s</font></td>
				<td><font size=1>%s</font></td>
				<td><font size=1>%s</font></td>
				<td><font size=1>%s</font></td>
				<td><font size=1><a target=new href='http://www.bzto.bz/bztobz8/RepActivity/ReviewTaskClient.aspx?prikey=%s'>%s</font></td>
				<td><font size=1>%s</font></td>
				<td><font size=1>%s</font></td>
				<td><font size=1>%s</font></td>
